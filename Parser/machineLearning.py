...
def JSONToCSV(workingDir):
    ...
    for file in fileList:
        if(file.find(".json")>0):
            dataCompleted = getJSON(workingDir+file)
            withoutextension = file.split(".json")
            createFileCSV(workingDir,withoutextension[0],dataCompleted,label_dict)
...
def clustering(workingDir):

    dataFrame = read_CSV(workingDir)
...
"""
def data_for_cluster(workingDir, name, new_df):
...
def apriori_algorithm(workingDir):
...
def new_apriori_algorithm(workingDir):
...
def silhouette_plots(data, labels, workingDir):
...
def labeled_IdSession(workingDir, label_dict):
...
def hyperparameter_UMAP(workingDir):
    data = read_CSV(workingDir)
    for n in (2, 5, 10, 20, 50, 100, 200):
        embbeding = umap.UMAP(n_neighbors=n).fit_transform(data)
        hyperparameter_kmeans(workingDir, embbeding, 'n_neighbors = {}'.format(n))
    ...
    for d in (0.0, 0.1, 0.25, 0.5, 0.8, 0.99):
        ...
        hyperparameter_kmeans(workingDir, embbeding, 'min_dist = {}'.format(d))
    ...
    for m in ("euclidean", "manhattan"):
        ...
        hyperparameter_kmeans(workingDir, embbeding, 'metric = {}'.format(name))
...
def hyperparameter_kmeans(workingDir, embbeding, title_umap):

    # Calcular previamente el metodo del codo para saber que valores indicar
    for n in (6, 7):
        draw_kmeans(workingDir, embbeding, n_clusters=n, title=title_umap+'n_clusters = {}'.format(n))

    for d in (10, 20, 30):
        draw_kmeans(workingDir, embbeding, n_init=d, title=title_umap+'n_init = {}'.format(d))

    for m in (300, 400, 500):
        draw_kmeans(workingDir, embbeding, max_iter=m, title=title_umap+'max_iter = {}'.format(m))


def draw_kmeans(workingDir, data, n_clusters = 7, init='k-means++', n_init=10, max_iter=300, title=''):
...
if __name__ == "__main__":
    ...
    if arg.dir is not None:
        ...
        #verify_label_classification(arg.dir,'knn')
        #separate_label_classification(arg.dir,'dt')
